...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    //prize add by lihuangyuan,for notscreen,2018-10-19-start
    static final Rect mTmpParentFrameCutEars = new Rect();
    static final Rect mTmpOverscanFrameCutEars = new Rect();
    //prize add by lihuangyuan,for notscreen,2018-10-19-end 
  ...
  /*PRIZE-three_slideshot-huangdianjun-2015-04-06-end*/
    ...
    public int checkAddPermission(WindowManager.LayoutParams attrs, int[] outAppOp) {
        int type = attrs.type;
        Log.d(TAG,"checkAddPermission: type = " + type);//mtk add
        ...
        if (permission != null) {
            if (android.Manifest.permission.SYSTEM_ALERT_WINDOW.equals(permission)) {
                ...
                final int mode = mAppOpsManager.checkOpNoThrow(outAppOp[0], callingUid,
                        attrs.packageName);
                Log.d(TAG,"checkAddPermission: mode = " + mode);//mtk add
                switch (mode) {
                    ...
                    case AppOpsManager.MODE_ERRORED:
                        try {
                            ...
                            Log.d(TAG,"checkAddPermission: appInfo.targetSdkVersion = " + appInfo.targetSdkVersion);//mtk add
                    ...
                    default:
                        ...
                        Log.d(TAG,"checkAddPermission: mContext.checkCallingPermission = " + mContext.checkCallingPermission(permission));//mtk add
            ...
            Log.d(TAG,"checkAddPermission: mContext.checkCallingOrSelfPermission(permission) = " + (mContext.checkCallingOrSelfPermission(permission)));//mtk add
    ...
    public int getNonDecorDisplayWidth(int fullWidth, int fullHeight, int rotation,
        ...
        if (mHasNavigationBar) {
            ...
            if (mNavigationBarCanMove && fullWidth > fullHeight) {
                ...
                //modify for soft keyboard show when cut lcd ears. replace the up line(android org code)  liyongli  2018/4/16 21:13
                int w = fullWidth - getNavigationBarWidth(rotation, uiMode);
                ...
                if( PrizeOption.PRIZE_NOTCH_SCREEN
                    ...
                    w -= mStatusBarHeight;
                    //Slog.d("lylkb", " debug stack  w="+w, new Throwable()); 
                }
...
// liyongli use bellow , replace  the line above (original code)
           ...
           layoutWallpaper(win, pf, df, of, cf);
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                if (attached != null) {
                    ...
                    if(false && PrizeOption.PRIZE_NOTCH_SCREEN 
                         && isAppWindow 
                        ...
                        boolean prizeNeedsmall = false;
                        boolean statusBarTransient = (sysUiFl & View.STATUS_BAR_TRANSIENT) != 0;
                        if( mStatusBar.isVisibleLw() ){
                            if((fl & FLAG_FULLSCREEN)!=0){
                                //Slog.v(TAG, " lyl -1- visble && FLAG_FULLSCREEN" );
                        ...
                        if( (sysUiFl & View.SYSTEM_UI_FLAG_IMMERSIVE)!= 0 
                            ||(sysUiFl & View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY)!= 0 
                        ){
                            //Slog.v(TAG, " lyl -3- IMMERSIVE" );
                            prizeNeedsmall = true; 
                        }
                        ...
                        if( prizeNeedsmall && attrs!=null  )
                        {
                            String str =  attrs.getTitle()+""; //power on, first run, str=[Starting com.android.mms]
                            //Slog.v(TAG, "-- lyltitle --  "+ str );
                            if( !str.startsWith("my.beautyCamera") //MeiRenXiangJi, no method fix , so the app not cut Lcd ears
                                 && !str.startsWith("com.youdao.dict")//fix 58340 com.youdao.dict/.activity.OCRFromCameraActivity
                                 && !str.startsWith("com.ss.android.ugc.live")
        ...
        if (PrizeOption.PRIZE_NOTCH_SCREEN) 
        {
            boolean isNeedAddTop=false;
            if((attrs.applicationInfoFlags & ApplicationInfo.FLAG_SYSTEM) == 0 //not system app 
                //&& (fl & WindowManager.LayoutParams.FLAG_FULLSCREEN) != 0 
                && (mDisplayRotation == Surface.ROTATION_0 || mDisplayRotation == Surface.ROTATION_180))
            {
            
                String str =  attrs.getTitle()+"";
                Slog.v(TAG, "layoutWindowLw  packages title =" + attrs.getTitle()+ "   sysUiFl=" +Integer.toHexString(sysUiFl)+",flag:"+Integer.toHexString(fl)) ; 
                
                if (isAppWindow1 && (fl & WindowManager.LayoutParams.FLAG_FULLSCREEN) != 0 ) 
                {
                    if ( ((fl & WindowManager.LayoutParams.FLAG_FULLSCREEN) != 0 ) &&
                        (fl&WindowManager.LayoutParams.FLAG_FORCE_NOT_FULLSCREEN) == 0 &&
                        ((sysUiFl & View.SYSTEM_UI_FLAG_IMMERSIVE)== 0 || 
                        (sysUiFl & View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY)== 0 ))
                    {  
                        isNeedAddTop=true;
                    }
                    ...
                    if(isNeedAddTop &&
                        (str.startsWith("com.mediatek.camera")||
                        //str.startsWith("com.tencent.mobileqq") ||
                        str.startsWith("com.tencent.mm") ))
                    {
                        isNeedAddTop=false;
                    }    
                }
                //kuwo music
                if(!isNeedAddTop && (str.startsWith("cn.kuwo.player")
                                                /*||str.startsWith("com.tencent.mobileqq/dov.com.tencent.biz.qqstory.takevideo.EditPicActivity")*/))
                {
                    isNeedAddTop = true;
                }
                ...
                if(isNeedAddTop)
                {
                    int cutHight = mStatusBarHeight - (mStatusBarHeight&0x01);
                    if(mDisplayRotation == Surface.ROTATION_0)
                    {
                        pf.top += cutHight;
                        of.top += cutHight;
                        ...
                        mTmpParentFrameCutEars.left = pf.left;
                        mTmpParentFrameCutEars.top = 0;
                        mTmpParentFrameCutEars.right = pf.right;
                        mTmpParentFrameCutEars.bottom = pf.bottom;
                        
                        mTmpOverscanFrameCutEars.left = of.left;
                        mTmpOverscanFrameCutEars.top = 0;
                        mTmpOverscanFrameCutEars.right = of.right;
                        mTmpOverscanFrameCutEars.bottom = of.bottom;
                        prizeCoverEarsWin.computeFrameLw(mTmpParentFrameCutEars, df, mTmpOverscanFrameCutEars, cf, vf, dcf, sf, osf);
                    }
                    else if(mDisplayRotation == Surface.ROTATION_180)
                    {
                            pf.bottom -= cutHight;
                        ...
                        && (fl&WindowManager.LayoutParams.FLAG_FORCE_NOT_FULLSCREEN) == 0)
                    {
                        if(isNeedAddTop)
                        {
                            win.setPrizeFlags(WindowManagerPolicy.WindowState.PRIZE_FLAG_CUT_SYSTEMUI);
                            //fill black block on the systemui
                        }
                        else
                        {
                            win.setPrizeFlags(WindowManagerPolicy.WindowState.PRIZE_FLAG_DONOTHING_SYSTEMUI);
                        }
                    }*/
                    //Slog.v(TAG, "-lhy--pf="+ pf.toShortString()+" of="+of.toShortString()+" mDisplayRotation="+mDisplayRotation+",PrizeFlag:"+win.getPrizeFlags()) ;                          
                }
            }

            if( mDisplayRotation == Surface.ROTATION_0
                && isAppWindow1 && (mFocusedWindow==win)            
                && attrs.type != TYPE_APPLICATION_STARTING 
                && !isNeedAddTop && coverEarsWinAttrs !=null  && coverEarsWinAttrs.y == 0  )
            {
                coverEarsWinAttrs.y = -100;
                if(DEBUG_NOTSCREEN)Slog.v(TAG, " layoutWindowLw  coverear  -100  win:" + win);
            }
        }
        //prize add by lihuangyuan,for notscreen,2018-10-19-end
        ...
        if(PrizeOption.PRIZE_NOTCH_SCREEN && attrsTitle!=null){
            if(attrsTitle.equals("CoverEars") || attrsTitle.equals("notchscreen")){
                ...
                pf.top = 0;
                pf.left = 0;                
            }else if(attrsTitle.equals("PointerLocation")){
                //fix bug,  Setting ->Developer options->Pointer location not draw in pen down position -liyongli-2018-07-19
                pf.top = 0;
                pf.left = 0;
                df.left = 0;
        ...
        if(PrizeOption.PRIZE_NOTCH_SCREEN && attrs.getTitle() != null && attrs.getTitle().equals("prizeDivMenu")){
...
//add by liup 20150519 for sleepgesture
    ...
    private void requestTransientBars(WindowState swipeTarget) {
        synchronized (mWindowManagerFuncs.getWindowManagerLock()) {
            ...
            Slog.d(TAG, "onSwipeFromTop() - isUserSetupComplete is true");
    ...
    public void systemBooted() {
        ...
        screenTurnedOn();
		/*prize-add by lihuangyuan,for prize reboot -2017-08-23-start*/
	 mHandler.postDelayed(new Runnable() {
                        public void run() {
                            checkPrizeReboot();
                        }
                    },5*1000);
    	  /*prize-add by lihuangyuan,for prize reboot -2017-08-23-end*/
    }
	
	/*prize-add by lihuangyuan,for prize reboot -2017-08-23-start*/
    private void checkPrizeReboot()
    {
        //PowerManager powerManager = (PowerManager)mSystemContext.getSystemService(Context.POWER_SERVICE);
       if(isPrizeBootMode())
       {
		mPowerManager.goToSleep(SystemClock.uptimeMillis(),
                                        PowerManager.GO_TO_SLEEP_REASON_POWER_BUTTON, 0);
    ...
    private boolean isPrizeBootMode()
    {
	    	File file = new File("/proc/prize_boot_mode");
		...
		try
		{
			...
			Slog.i(TAG,"isPrizeBootMode :"+str);
			if("1".equals(str))
			{
				return true;
			}
...
//                    updateNavbarColorLw(winNavBar, visibility, false);
    ...
    /**
     * prize add by xiarui PRIZE_BARRAGE_WINDOW 20190513 start
     * @return isImmersiveMode, include navgation bar judge
     */

    private String[] mExcludeApps = {
            "com.mediatek.camera",
            "com.android.camera",
            "com.android.systemui"
    };

    private boolean isExcludeBarrage() {
        //String topPackageName = getTopActivity().getPackageName();
        String topApp = getTopApp();
        for (int i = 0; i< mExcludeApps.length; i++) {
            //if (topPackageName != null && mExcludeApps[i].contains(topPackageName)) {
            if (topApp != null && topApp.contains(mExcludeApps[i])) {
                return true;
            }
        }
        return false;
    ...
    public boolean isInImmersiveMode() {
        //return isImmersiveMode(mLastSystemUiFlags);
        if (mStatusBar != null && !mStatusBar.isVisibleOrBehindKeyguardLw() && !isExcludeBarrage()) {
            return true;
        }
        return false;
    }
    //prize add by xiarui PRIZE_BARRAGE_WINDOW 20190513 end
}
